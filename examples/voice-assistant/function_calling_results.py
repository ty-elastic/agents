...
class AssistantFnc(llm.FunctionContext):
    def __init__(self, *, light_state: Dict[Room, bool]):
        super().__init__()
    ...
    async def get_light_status(
        ...
        if room in self._light_state:
            logging.info("get_light_status %s %s", room, self._light_state[room])
            return self._light_state[room]
        else:
            return False
...
async def entrypoint(ctx: JobContext):
    ...
    light_state = {}
    light_state[Room.OFFICE.value] = True
    ...
    assistant = VoiceAssistant(
        ...
        llm=gpt,
        tts=azure.TTS(),
        fnc_ctx=AssistantFnc(light_state=light_state),
